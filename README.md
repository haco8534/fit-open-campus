...
2. `public/slides/1.png` 〜 `8.png` を置き換える（スライド番号とは1つずれる。
   アプリ側で描画する待機画面がスライド1のため。別の拡張子にする場合は
...
| スライド | メインモニター | 参加者画面 |
|---|---|---|
| 1 | 開始前の案内（大きいQR・準備中アニメ） | 待機のみ。**何も送れない** |
| 2〜7 | スライド画像 | リアクション＋コメント |
| 8 | 投票ボード | 投票 |
| 9 | スライド画像 | お礼＋質問フォーム＋リアクション＋コメント |

スライド1と8は画像ではなくアプリ側で描画している（`public/slides/` に対応する画像は無い）。
開場中はスライド1を出しておけば、来場者はQRから先に参加登録だけ済ませられる。
...
- **スライド8に入った瞬間**が投票の開始であり、同時に前の票のリセットでもある
