...
語の一覧は [src/lib/ngWords.ts](src/lib/ngWords.ts)、判定ロジックは
[src/lib/moderation.ts](src/lib/moderation.ts)。約380語を登録している。
一部は [MosasoM/inappropriate-words-ja](https://github.com/MosasoM/inappropriate-words-ja)（MIT）から取り込んだ。

判定前に本文を正規化するので、次のような回避は効かない。

| 入力 | 判定 |
|---|---|
| `し ね` `し・ね` `し～ね` | 空白と記号を落としてから照合 |
| `ｼﾈ` `ＳＥＸ` | NFKCで半角カナ・全角英数を揃えてから照合 |
| `シネ` | カタカナをひらがなに畳んでから照合 |

語は2つのリストに分けている。

- `NG_WORDS_FOLDED`（3文字以上／漢字を含む語）— かなを畳んで部分一致
- `NG_WORDS_EXACT`（かな2文字）— **畳まずに**部分一致。
  畳むと「イク」が「行く」に、「カス」が「動かす」に反応してしまうため

`SAFE_PHRASES` は「ばかり」「激しい」「イクラ」のように、NGワードを含むが
問題のない語。判定前に本文から取り除いている。

語を足すときは `ngWords.ts` を直接編集する。**漢字表記は別途登録が必要**
（畳み込みはカタカナ→ひらがなのみで、漢字は変換されないため）。

> ⚠ このフィルタはブラウザ側で動く。DBに直接書き込まれた場合は素通りするので、
> 最後の砦は管理画面の「コメント全削除」と「コメント受付OFF」。
