...
| キー | 動作 |
|---|---|
| ← / → | スライド移動（モード固定中でなければ参加者画面のモードも切り替わる） |
| P | 投票開始 / 終了 |
| C | コメント受付 ON/OFF |
| R | リアクション受付 ON/OFF |
...
「参加者画面モード」のボタンを押すと、そのモードは **🔒 手動固定** になり、
以降スライドを動かしても参加者画面のモードは変わらない。
（質疑応答中に「次へ」を押して参加者の質問フォームが消える、といった事故を防ぐため）
スライド追従に戻したいときは、同じパネルの「スライド追従に戻す」を押す。

- スライド7（トークテーマ）に移動 →「投票開始」→ 参加者のスマホに投票画面が自動表示
  （投票中はモードが自動で固定される。どのスライドで開始してもスクリーンに投票画面が出る）
